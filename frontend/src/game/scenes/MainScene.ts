import { Scene, vec, Vector } from "excalibur";
import { Resources } from "../config/resources";
import { Player } from "../actors/Player";
import { OtherPlayer } from "../actors/OtherPlayer";
import { PlayerData, SceneCallbacks } from "@/types/datatypes";
import { getStompClient } from "@/hooks/useStompClient";
import { Client } from "@stomp/stompjs";
import { getPlayers } from "@/utils/api";

export class MainScene extends Scene {
  private player!: Player;
  private otherPlayers: Record<string, OtherPlayer> = {};
  // private callbacks: SceneCallbacks;
  private stompClient: Client | null = getStompClient();
  private roomId: string;
  private userId: string;
  private username: string;
  private lastBroadcast = 0;
  private BROADCAST_INTERVAL = 1000; // ms


  private subscriptionId?: string;

  constructor(callbacks: SceneCallbacks = {}, userId: string, roomId: string, username: string) {
    super();
    // this.callbacks = callbacks;
    console.log(callbacks);
    this.userId = userId;
    this.roomId = roomId;
    this.username = username;
  }

  async onInitialize(): Promise<void> {
    Resources.LdtkResource.addToScene(this, {
      pos: vec(0, 0),
      levelFilter: ["Level_0"]
    });
    this.findEntities();
    await this.initPlayers(this.roomId);
    this.setupSubscription();
  }

  private findEntities() {
    this.player = this.world.entityManager.getByName(this.username)[0] as Player;
  }

  onPostAdd() {
    this.findEntities();
  }

  onActivate(): void {
    if (this.player) {
      this.camera.strategy.lockToActor(this.player);
      const bounds = Resources.LdtkResource.getLevelBounds(["Level_0"]);
      this.camera.strategy.limitCameraBounds(bounds);
    }
  }

  onPreUpdate(): void {
    const now = Date.now();
    if (this.player && this.stompClient?.connected) {
      if (now - this.lastBroadcast > this.BROADCAST_INTERVAL) {
        const positionMessage: PlayerData = {
          user_id: this.userId,
          room_id: this.roomId,
          name: this.username,
          avatarId: this.player.avatarId,
          x: this.player.pos.x,
          y: this.player.pos.y,
          id: "", // the id is not needed, in the backend, we need to let psql generate this
          direction: this.player.currentDirection
        };
        this.stompClient.publish({
          destination: "/app/game/position",
          body: JSON.stringify(positionMessage),
        });
        this.lastBroadcast = now;
      }
    }
  }

  private async initPlayers(roomId: string) {
    const players = await getPlayers(roomId);
    if (!players) return;

    players.forEach(playerData => {
      const pos = vec(playerData.x, playerData.y);
      if (playerData.user_id === this.userId) {
        this.player.pos = pos;
      } else {
        const other = new OtherPlayer({
          pos,
          z: 15,
          width: 16,
          height: 16,
          anchor: new Vector(0.5, 0.5),
          userId: playerData.user_id,
          roomId: playerData.room_id,
          name: playerData.name,
          avatarId: playerData.avatarId,
        });
        this.otherPlayers[playerData.user_id] = other;
        this.add(other);
      }
    });
  }

  private setupSubscription() {
    if (!this.stompClient) {
      console.error("STOMP client not initialized!");
      return;
    }

    // Subscribe to the room topic
    const subscription = this.stompClient.subscribe(`/topic/game/${this.roomId}`, (message) => {
      const data: PlayerData = JSON.parse(message.body);
      if (data.user_id === this.userId) return; // ignore own message

      const pos = vec(data.x, data.y);
      let other = this.otherPlayers[data.user_id];
      if (other) {
        other.walkTo(pos);
      } else {
        other = new OtherPlayer({
          pos,
          z: 15,
          width: 16,
          height: 16,
          anchor: vec(0.5, 0.5),
          userId: data.user_id,
          roomId: data.room_id,
          name: data.name,
          avatarId: data.avatarId,
        });
        this.otherPlayers[data.user_id] = other;
        this.add(other);
      }
    });

    this.subscriptionId = subscription.id;
  }

  onDeactivate(): void {
    if (this.stompClient && this.subscriptionId) {
      this.stompClient.unsubscribe(this.subscriptionId);
    }
    this.otherPlayers = {};
  }
}
