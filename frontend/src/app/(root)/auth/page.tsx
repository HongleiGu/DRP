...
import { verifyOtp, sendOtp } from "@/utils/user";
...
export default function AuthPage() {
  ...
  const onVerify = async (values: { token: string }) => {
    ...
    try {
      ...
      } as SupabaseUser, password)
      await globalStore.setItem('lumiroom-user', JSON.stringify({
        id: "", // id is generated by supabase, cannot preset
        email,
        username,
        avatar_id: parseInt(avatarId),
  ...
  const onLogin = async (values: SignInArgs) => {
    ...
    try {
      ...
      const resUser = await signIn({ email: emailToUse, password: values.password });
      if (resUser) {
        setUser(resUser);
        await globalStore.setItem('lumiroom-user', JSON.stringify(resUser))
