...
export default function FestivalCalendar({
  ...
  const renderDateCell = useCallback(
    (date: Dayjs) => {
      ...
      return (
        <div
          className={`flex flex-col p-1 gap-1 cursor-pointer rounded-lg transition ${
            dayEntries.length ? "bg-gray-50 border" : "bg-gray-50"
          } ${isToday ? "ring-2 ring-blue-300" : ""} hover:shadow-md`}
            ...
            <div className="flex gap-3">
              {user && (
                <div className="bg-gray-100 px-3 py-1.5 rounded-full flex items-center gap-2">
                  ...
                  <Text>
                    {(user.publicMetadata.nickname as string) ?? "Anonymous"}
                  </Text>
                </div>
              )}
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                    ...
                    <div className="max-h-48 overflow-y-auto grid grid-cols-6 gap-1 p-2">
                      {ALL_EMOJIS.flatMap((g) => g.emojis).map((emoji) => (
                        <span
                          ...
                          onClick={() =>
                            setCurrentFestival((prev) => ({ ...prev, emoji }))
                          }
                        >
                          {emoji}
                        </span>
                  ...
                  <Button
                    ...
                    icon={
                      currentFestival.emoji ? (
                        <span className="text-xl">{currentFestival.emoji}</span>
                      ) : (
                        <PlusOutlined />
                      )
                    }
                ...
                <Button
                  ...
                  disabled={!currentFestival.name || !currentFestival.emoji}
                >
                  Add
                </Button>
              </div>
